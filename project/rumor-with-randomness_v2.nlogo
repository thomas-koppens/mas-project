...
patches-own [
  has-heard    ;; tracks whether the patch has heard the rumour or not
...
to seed-random
  ...
  ask patches with [has-heard = false] [
...
to go
  if all? patches [has-heard = true] [ stop ]
  ask patches with [ has-heard = true ] [ spread-rumor ]
  tick
end
...
to spread-rumor  ;; patch procedure
  let neighbor nobody
  let trials 0
  let suitable-neighbor false

  while [not suitable-neighbor and trials < 8] [
    set trials trials + 1
    ifelse eight-mode? [
    ...
    if [has-heard] of neighbor = false [
      set suitable-neighbor true
    ]
  ]

  ;; Spread the rumor to a suitable neighbor if one was found
  if suitable-neighbor and random-float 1 <= spread-chance [
...
"" ""
PENS
"default" 1.0 0 -2674135 true "" "ifelse previous-clique = 0 \n[\n    plot 1\n] \n[\nplot clique / previous-clique\n]"
