turtles-own [
  has-heard?     ; Boolean
  popularity     ; Float
  familiarities  ; List[List[Turtle, Float]]
  avg-familiarity
]
...
to hear-rumor  ; update the associated variable and colour to represent knowing the rumor
  set has-heard? true
  set color red
end
...
to setup
  ...
  crt 100 [
    ...
    set popularity random-float 1  ; Initialise turtles' popularity between 0.0 and 1.0
    ...
    seed-rumor                     ; Tell the rumor to a single turtle
  ]
...
to setup-familiarities
  ...
  ask turtles [
    ...
    ask other turtles in-radius search-distance [
      let distance-to-other distance myself
      let normalised-distance distance-to-other / search-distance
    ...
    set avg-familiarity 0
    foreach familiarities [tuple -> set avg-familiarity avg-familiarity + item 1 tuple]
    set avg-familiarity avg-familiarity / length familiarities
  ]
end
...
to update-familiarity [original-turtle]
  let familiarity-tuple filter [x -> first x = original-turtle] familiarities
  ifelse empty? familiarity-tuple [
    set familiarities lput (list original-turtle (0 + familiarity-inc)) familiarities
  ] [
    set familiarity-tuple item 0 familiarity-tuple
    let index position familiarity-tuple familiarities
    set familiarity-tuple replace-item 1 familiarity-tuple min list 1 (item 1 familiarity-tuple + familiarity-inc)
    set familiarities replace-item index familiarities familiarity-tuple
    foreach familiarities [tuple -> set avg-familiarity avg-familiarity + item 1 tuple]
    set avg-familiarity (sum item 1 familiarities) / (length familiarities)
  ]
end
...
to-report valid-spread? [original-turtle other-turtle]
  ...
  report random-float 1 <= calculate-probability popularity familiarity
end
...
to-report calculate-probability [pop fam]
  ...
  let total sum list weighted-pop weighted-fam

  ; Calculate the probability using the logistic function
  let probability 1 / (1 + exp(- (spread-chance + total)))
...
to-report mean-familiarity
  let n-fams 0
  let fam 0

  ask turtles [
    foreach familiarities [
     tuple -> set fam fam + item 1 tuple
     set n-fams n-fams + 1
    ]
  ]
...
to-report avg
  let total-fam []
 ask turtles [
    set total-fam lput avg-familiarity total-fam
  ]
 report total-fam
end
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot mean-familiarity"
...
"" ""
PENS
"pen-0" 1.0 0 -7500403 true "" "plot mean avg"
"pen-1" 1.0 0 -2674135 true "" "plot median avg"
