globals [og-rumor group-centres]
...
to setup
  ...
  foreach group-centres [
    centre -> ask centre [set pcolor item (position centre group-centres + 9) base-colors]
  ]
  setup-positions
  setup-familiarities
...
to assign-groups
  ...
  let max-group-size floor n-turtles / n-groups

  let current-group-id 0
  let current-group-size 0

  ask turtles [
    ifelse current-group-size < max-group-size [
    ] [
      if current-group-id < (n-groups - 1) [
        set current-group-id current-group-id + 1
        set current-group-size 0
      ]
    ]
    set group-id current-group-id
    set color item (group-id + 9) base-colors
...
to move
  ...
  ask turtles [
    ...
    let random-angle random 360  ; Generate a random angle

    if dist-to-centre > r [ ; If we are out of our group, maybe go back towards it
      facexy [pxcor] of respective-centre [pycor] of respective-centre + random-angle
    ]

    ; Move forward while checking boundaries
    let new-xcor xcor + 1 * cos random-angle
    let new-ycor ycor + 1 * sin random-angle

    if not (abs new-xcor > max-pxcor or abs new-ycor > max-pycor) [
...
to-report distributed-centres [n-centres]
  let radius 14
  let theta 360 / n-centres

  let centres []
  let index 0

  while [index < n-centres] [
   let ang index * theta + 90   ; shifting by 90deg makes it look better
   ; x = rcos(theta), y = rsin(theta)
   let x radius * cos ang
   let y radius * sin ang

   let respective-patch patch x y
   set centres lput respective-patch centres
